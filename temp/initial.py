...
class Initial:
    def __init__(self, data_path):
        self.initial = {'data_path': data_path,
                       'LinearRegression': {
                       ...
                       'Ridge': {
                           'model': Ridge(),
                                 'param_grid': {'alpha': [i + 0.0001 for i in range(0, 20)],
                                                'max_iter': [1000000],
                                                'solver' : ['auto']
                                 }},
                       ...
                       'Lasso': {
                           'model': Lasso(),
                                 'param_grid': {'alpha': [0.1, 1, 5, 10, 20, 30, 40, 50, 100, 500, 1000],
                                                'max_iter': [1000000]},
                                 },
                       'Elastic': {
                           'model': ElasticNet(),
                                   'param_grid': {'alpha': [0.1, 1, 5], 'l1_ratio': [0.1, 0.5, 0.7, 0.9, 0.95, 1.0],
                                                  'max_iter': [1000000]}
                       },
                        'K_Neighbours': {
                            'model': KNeighborsRegressor(),
                                        'param_grid': {'n_neighbors': [i for i in range(1, 30)], 'algorithm': ['auto']}
                        },
                       'DecisionTree': {
                           'model': DecisionTreeRegressor(),
                                        'param_grid': {'max_leaf_nodes': [100]}
                       },
                       'RandomForestRegressor': {
                           'model': RandomForestRegressor(),
                                                 'param_grid': {'criterion': ['squared_error'], 'max_depth': [100],
